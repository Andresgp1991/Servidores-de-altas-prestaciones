...
![Imagen](https://github.com/Andresgp1991/Servidores-web-de-altas-prestaciones/blob/master/Practica5/Imagenes/Figura4.png "Master status")
...
![Imagen](https://github.com/Andresgp1991/Servidores-web-de-altas-prestaciones/blob/master/Practica5/Imagenes/Figura5.png "Replicación maestro-esclavo")

## Replicación de BD mediante una configuración maestro-maestro

### Paso 1. Configurar MySQL en el Servidor web 1

Para ello editamos el fichero de configuración **/etc/mysql/my.cnf**, en que cambiaremos las siguientes líneas como en el apartado anterior:
...
Nos conectamos a nuestro servidor de base de datos mediante la siguiente instrucción:

**mysql -u root -p**

Lo que debemos hacer es crear un usuario que será utilizado para la replicación de los datos entre nuestros dos servidores. Para crearlo ejecutamos lo siguiente sentencia:

**create user 'esclavo'@'%' identified by 'esclavo';**

Lo siguiente será darle permisos a ese usuario para que pueda realizar la replicación de los datos:

**grant replication slave on \*.\* to 'esclavo'@'%' identified by 'esclavo';**

Para finalizar este primer paso, es necesario recopilar cierta información que luego será necesaria para la configuración del segundo servidor. Para ello ejecutamos la siguiente instrucción:
...
### Paso 2. Configurar MySQL en el Servidor web 2

En este segundo paso, necesitamos repetir los mismos pasos que realizamos en el punto anterior con la única diferencia que a la hora de editar el fichero **/etc/mysql/my.cnf**, en la directiva *server-id*, no podremos utilizar el valor 1 ya que fue el que indicamos en el Servidor web 1. Para este caso utilizaremos el valor 2.

Después de hacer esto, reiniciamos el servidor MySQL y repetimos los mismos pasos:
...
El siguiente paso consiste en utilizar la información que anotamos en el paso anterior y aplicar a nuestra instancia MySQL. Esto permitirá que comience la replicación de los datos. Para realizar esto, debemos ejecutar la siguiente instrucción desde consola:

**slave stop;**

**change master to master_host='172.16.186.130', master_user='esclavo', master_password='esclavo', master_log_file='mysql-bin.000002', master_log_pos=355, master_port=3306;**

**start slave**

		slave stop;
		Query OK, 0 row affected, 1 warning (0.00 sec) 
		mysql> change master to master_host='172.16.186.130', master_user='esclavo', master_password='esclavo', master_log_file='mysql-bin.000002', master_log_pos=355, master_port=3306;
		Query OK, 0 row affected (0.07 sec) 
		slave start;
		Query OK, 0 row affected (0.00 sec) 

### Paso 3. Completar proceso de replicación en Servidor web 1

Volvemos al Servidor web 1 para finalizar la configuración. Para ello desde línea de comandos ejecutamos las siguientes sentencias:

**slave stop;**

**change master to master_host='172.16.186.131', master_user='esclavo', master_password='esclavo', master_log_file='mysql-bin.000002', master_log_pos=1234, master_port=3306;**

**start slave**

		slave stop;
		Query OK, 0 row affected, 1 warning (0.00 sec) 
		mysql> change master to master_host='172.16.186.131', master_user='esclavo', master_password='esclavo', master_log_file='mysql-bin.000002', master_log_pos=355, master_port=3306;
		Query OK, 0 row affected (0.07 sec) 
		slave start;
		Query OK, 0 row affected (0.00 sec)

### Paso 4. Testear la replicación maestro-maestro

Vamos a crear una tabla en nuestra base de datos en el Servidor web 2 y comprobar en el Servidor web 1 a ver si aparece y viceversa.

Como ya teniamos creadas las bases de datos *Usuarios* con la tabla *datos* del apartado anterior son las que vamos a usar, para ello añadimos los siguientes datos como vemos en la Figura1 y Figura2:

![Imagen](https://github.com/Andresgp1991/Servidores-web-de-altas-prestaciones/blob/master/Practica5/Imagenes/Figura6.png "Figura1: Servidor web 1") ![Imagen](https://github.com/Andresgp1991/Servidores-web-de-altas-prestaciones/blob/master/Practica5/Imagenes/Figura7.png "Figura2: Servidow web 2")
